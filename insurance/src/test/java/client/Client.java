...
public class Client {
	...
	public static void main(String[] args) throws Exception {
		...
		// Client client = new Client("INSUR01 ", "n.nn.nnn.nnn", "dataStore", 8890,
		// "racfId", "racfGroupName", "CLIENX1");

		Client client = new Client("YPM130MA", "19.5.34.146", "", 3008, "FCHBK81", "", "");
	 ...
	 * 
	 * @param tranName
	 *            8 character transaction name
	 * @param host
	 *            the IP address of the z/OS system
	 * @param dataStore
	 *            the IMS data store name
	 * @param port
	 *            the IMS Connect port
	 * @param racfId
	 *            8 character RACFID
	 * @param racfGroupName
	 *            8 character RACF GROUP ID
	 * @param clientId
	 *            8 character IMS Connect client ID
	 */

	public Client(String tranName, String host, String dataStore, int port, String racfId, String racfGroupName,
			String clientId) {
	...
	public void invoke() throws ImsConnectApiException, Exception {
		...
		myTmInteraction.setInputMessageDataSegmentsIncludeLlzzAndTrancode(
				ApiProperties.INPUT_MESSAGE_DATA_SEGMENTS_DO_NOT_INCLUDE_LLZZ_AND_TRANCODE);
	...
	/**
	 * Creates a byte array from a Vector of byte array elements. The byte array
	 * will be sent to the specified IMS transaction.
	 * 
	 * @return byte array
	 * 
	 */
